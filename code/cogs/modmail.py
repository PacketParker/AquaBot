...
class ModMail(commands.Cog):
    ...
    async def contact(self, ctx):
        embed = nextcord.Embed(
            title = "Disabled",
            description = f"Modmail/Contact feature has been temporarily deisabled while I work on ironing out the bugs. For any urgent issues, please join the support server and ping the owner. Do `{ctx.prefix}join` to get the invite.",
            colour = nextcord.Colour.random()
        )            
