...
class Slots(commands.Cog):
    ...
    @commands.command()
    @commands.cooldown(1, 1.5, type=commands.BucketType.user)
    #Slot machine. Bet must be between 1 (default) - 3
    #Usage: 'slots [bet]'
    async def slots(self, ctx: commands.Context, bet: int=0):
        bet = int(bet)
        if bet <= 0 or bet > 3:
        ...
        if bet < current and bet == 1 or bet == 2 or bet == 3:
    ...
    async def slots_error(self, ctx, error):
        if isinstance(error, commands.errors.BadArgument):
            embed = nextcord.Embed(
                colour = color,
                title = "Bet too large/small!",
                description = "You must set your bet at either 1, 2, or 3. Example: `$slots <amount (1,2,3)>`"
            )
        ...
        if isinstance(error, InsufficientFundsException):
            embed = nextcord.Embed(
                colour = color,
                title = "No credits!",
                description = "You do not have any credits to use on slots. Try buying some using\n`$buyc <amount>`, or earn some more money at the blackjack tables."
        ...
        if isinstance(error, commands.CommandOnCooldown):
            embed = nextcord.Embed(
                ...
                description = "In order to keep up with the creation of gifs, the `$slots` command is at a 2 second cooldown."
            )
            embed.set_footer(text=datetime.now().strftime("%m/%d/%Y %H:%M:%S"))
            await ctx.send(embed=embed)
