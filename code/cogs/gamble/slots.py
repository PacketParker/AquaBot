...
class Slots(commands.Cog):
    ...
    async def slots(self, ctx: commands.Context, bet: int=1):
        ...
        embed = make_embed(
            title=(f'You {result[0]} {result[1]} credits'+ ('.' if result[0] == 'lost' else '!')), # happy or sad based on outcome
            description=('You now have ' + f'**{self.economy.get_entry(ctx.author.id)[2]}** ' + 'credits.'),
            color=(nextcord.Color.red() if result[0] == "lost" 
                else nextcord.Color.green()
            )
        )

        file = nextcord.File(fp, filename=fp)
        embed.set_image(url=f"attachment://{fp}") # this doesn't make any sense to me
        await ctx.send(
            file=file,
            embed=embed
        )
